...
const BlogCard = ({ image, title, description, category, readTime }) => {
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300 border border-gray-200 animate__animated animate__fadeInUp flex flex-col h-full">
      <img
        src={image}
        alt={title}
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className="mt-auto">
          <Link
            to="/blogdetail"
            className="bg-[#f6b60d] hover:bg-[#d99d0c] text-white px-4 py-2 rounded-lg shadow-md"
          >
            Read Article
          </Link>
        </div>
      ...
      <div className="space-y-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {blogs.slice(0, visibleBlogs).map((blog, index) => (
            <div key={blog.id} className={`animate__animated animate__fadeInUp animate__delay-${index}s flex items-stretch`}>
              <BlogCard
                image={blog.image}
                title={blog.title}
